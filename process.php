	...
	if($vpb_cleaned_domain_name_to_search != "")
	{
		foreach($vpb_domain_servers as $vpb_domain_servers_ext => $vpb_domain_whois)
		{
			...
			if($vpb_socket = fsockopen($vpb_domain_whois[0], 43))
			{
				...
				{
					echo '<div id="vasplus_pb">
					<div class="available">
					<form id="myForm" method="POST" action="/domain">
					<div style="float:left;  border:0px solid; padding-top:3px; padding-bottom:2px;">
					<span>Available</span><input type="text" style="border:none; outline:none; padding:0px; margin:0px; " name="domain" value="' . $vpb_cleaned_domain_name_to_search. '' . $vpb_domain_servers_ext .'"></b> is Available
					</div>
					
					<div style="float:right;" align="right">
					<button type="submmit" class="vpb_general_button" style="color:#FFF; padding:3px; font-family:Verdana, Geneva, sans-serif; font-size:11px; cursor:pointer; margin:0px;" onclick="vpb_buy_now(\''.$vpb_cleaned_domain_name_to_search.$vpb_domain_servers_ext.'\')">
					Buy Now
					</button>
					</div>
					</form> 
